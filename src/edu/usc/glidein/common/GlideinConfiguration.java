 ...
 *   <li>$GLIDEIN_HOME/etc/glidein.config</li>
 ...
 */
public class GlideinConfiguration
{
	/**
	 * The default name of the glidein configuration file
	 */
	public static final String CONFIG_FILE = "glidein.config";
	
	/**
	 * Singleton instance
	 */
	private static GlideinConfiguration instance;
	 ...
	 */
	private GlideinConfiguration(File configFile)
	 ...
	 */
	public static synchronized GlideinConfiguration getInstance()
	throws GlideinException
	{
		if(instance==null)
		{
			...
			if(sys!=null && sys.length()>0)
			{
				...
				if(sysConfig.exists())
				{
					instance = new GlideinConfiguration(sysConfig);
					return instance;
				}
			}
			...
			if(env!=null && env.length()>0)
			{
				...
				if(envConfig.exists())
				{
					instance = new GlideinConfiguration(envConfig);
					return instance;
				}
			}
			
			// 3. Try $GLIDEIN_HOME/etc/glidein.config
			String glideinHome = System.getenv("GLIDEIN_HOME");
			if(glideinHome!=null && glideinHome.length()>0){
				File globusFile = new File(
						glideinHome + File.separator +
						"etc" + File.separator +
						CONFIG_FILE);
				if(globusFile.exists())
				{
					instance = new GlideinConfiguration(globusFile);
					return instance;
				}
			}
			
			// 4. Try ${user.home}/.glidein/glidein.config
			...
			if(home!=null && home.length()>0)
			{
				...
				{
					instance = new GlideinConfiguration(homeConfig);
					return instance;
				}
			}
			
			// 5. Try $GLOBUS_LOCATION/etc/glidein_service/glidein.config
