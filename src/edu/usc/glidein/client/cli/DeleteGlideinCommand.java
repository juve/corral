...
public class DeleteGlideinCommand extends Command
{
	private Options options = null;

	@SuppressWarnings("static-access")
	public DeleteGlideinCommand()
	{
		options = new Options();
		options.addOption(
		...
		for (String arg : args) {
			if (arg.matches("[1-9][0-9]*")) {
				try {
					...
					if (isDebug()) System.out.print("Deleting glidein "+id+"... ");
					glidein.delete(new EmptyObject());
					if (isDebug()) System.out.println("done.");
				} catch (Exception e) {
					System.out.println("Unable to delete glidein '"+arg+"': "+e.getMessage());
	...
	public String getHelp()
	{
		...
		buff.append("delete-glidein (dg): Delete an existing glidein\n");
		buff.append("usage: delete-glidein ID...\n");
		buff.append("\n");
